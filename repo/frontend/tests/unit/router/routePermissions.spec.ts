...
// Mock vue-router's createRouter and createWebHistory to avoid DOM/history issues in tests.
// We capture the routes configuration passed to createRouter.
...
// Mock the authGuard so the import does not fail
...
describe('Route Permissions Configuration', () => {
  ...
  beforeEach(async () => {
    // Dynamic import triggers the mock, capturing routes
    vi.resetModules()
